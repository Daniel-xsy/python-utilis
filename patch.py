###############################################################
#
# This file crop image to certain size to do data augmentation
#
# pkl Example:
...
patch_img_save_file = '../patch/images_%s/'%str(mode)
patch_annot_save_file = '../patch/patch_%s_train.pkl'%str(mode)
patch_h, patch_w = 416, 416
save_img = True



### add new perameters 'mode' to choose patch mode e.g center/ratio/inside
def Img_Patch(img_prefix, annot_dict, patch_w=640, patch_h=640, save_img=True,mode='center'):
	'''
	args:
		mode: 'center': choose labels whose center locates in the new img
			  'intersec': choose labels which exist intersection with the patch area
			  'ratio': choose labels whose iou > ratio with a patch area
	'''
	## padding means
	...
	if save_img:
		...
		if img_h!=h or img_w!=w:
			raise print('height or width mismatch in annotaion dict and image')
	...
	## has ignore region in annotation
		...
		for j in range(num_h):
			...
			## clip bbox into image_size and grid of area=0
				...
				current_an = current_an[select_index]
    				...
    				raise print('Mode Error, choice: center/intersec')
...
if __name__=='__main__':
	...
	## load data information from pkl file
	...
	for num, annot_dict in enumerate(inf):
		...
		if save_img:
			patch_img, patch_annots = Img_Patch(img_prefix, annot_dict, patch_w=patch_w, 
												patch_h=patch_h, save_img=save_img,mode=mode)
		else:
			patch_annots = Img_Patch(img_prefix, annot_dict, patch_w=patch_w, 
										patch_h=patch_h, save_img=save_img,mode=mode)
